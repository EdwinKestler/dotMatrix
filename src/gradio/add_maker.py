...
def dot_matrix_effect(frame, dot_size):
    ...
    # Define cell size (grid) as a multiple of the dot size.
    cell_size = int(dot_size * 4)
...
def convert_media(input_media, dot_size):
    ...
    if any(filename.endswith(ext) for ext in [".mp4", ".avi", ".mov", ".mkv"]):
        ...
        # Return the same video path twice: one for display and one for storing in state.
...
with gr.Blocks() as demo:
    ...
    gr.Markdown(
        ...
        "Use the **Replay Video** button to replay the processed video."
    )
    
    with gr.Row():
        # Updated file_types to include leading dots.
        input_video = gr.File(label="Upload Video", file_types=[".mp4", ".avi", ".mov", ".mkv"])
        dot_slider = gr.Slider(minimum=1, maximum=20, step=1, value=3, label="Dot Size (radius in pixels)")
    ...
    # When the process button is clicked, process the video and update the state.
